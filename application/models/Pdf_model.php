    ...
    public function fetch_shipping_label($shipping_no, $order, $items_arr, $address_obj, $order_type_label, $logo_url, $barcode_url, $type = "self", $ship_order_id = null)
    {
        ...
        if (!empty($barcode) || !empty($qr_code)) {
            ...
            $output .= '<table id="order_invoice_" class="table table-bordered" style="width: 100%; margin-top: 10px; border-collapse: collapse;">
                    <thead>
                        <tr style="background-color: #f0f0f0;">
                            ...
                            <th class="text-left" style="border: 1px solid #000; padding: 8px; width: 30%;">Product</th>
                            <th class="text-left" style="border: 1px solid #000; padding: 8px; width: 10%; text-align: center;">Qty</th>
                            <th class="text-left" style="border: 1px solid #000; padding: 8px; width: 15%;">' . ($order_type_label == 'Bookset' ? 'Product Code' : 'SKU') . '</th>
            ...
            if ($order_type_label == 'Bookset' && $this->db->table_exists('tbl_order_bookset_products')) {
                ...
                if (!empty($order_id_for_bookset)) {
                    ...
                    if (!empty($bookset_products) && count($bookset_products) > 0) {
                        ...
                        $output .= '<tr>
                                    <td class="text-left" style="border: 1px solid #000; padding: 8px; background-color: #e0e0e0; font-weight: bold; font-size: 1.05em;" colspan="6">
                        ...
                        foreach ($packages as $package_id => $package_data) {
                            // Package header row
                            $output .= '<tr>
                                    <td class="text-left" style="border: 1px solid #000; padding: 8px; background-color: #f0f0f0; font-weight: bold;" colspan="6">
                            ...
                            foreach ($package_data['products'] as $bookset_product) {
                                ...
                                $output .= '<tr>
                                        ...
                                        <td class="text-left" style="border: 1px solid #000; padding: 8px; text-align: center;">' . (isset($bookset_product->quantity) ? intval($bookset_product->quantity) : 1) . '</td>
                        ...
                        foreach ($items_arr as $item) {
                            ...
                            $output .= '</td>
                                    <td class="text-left" style="border: 1px solid #000; padding: 8px; text-align: center;">' . (isset($item->product_qty) ? intval($item->product_qty) : 1) . '</td>
                ...
                foreach ($items_arr as $item) {
                    ...
                    $output .= '</td>
                            <td class="text-left" style="border: 1px solid #000; padding: 8px; text-align: center;">' . (isset($item->product_qty) ? intval($item->product_qty) : 1) . '</td>
                            <td class="text-left" style="border: 1px solid #000; padding: 8px;">' . ($order_type_label == 'Bookset' ? (!empty($item->model_number) ? htmlspecialchars($item->model_number) : (!empty($item->product_sku) ? htmlspecialchars($item->product_sku) : '')) : (!empty($item->product_sku) ? htmlspecialchars($item->product_sku) : '')) . '</td>
                            <td class="text-left" style="border: 1px solid #000; padding: 8px;">';
            ...
            $output .= '<table id="invoice" class="" style="width: 100%; margin-top: 15px;">
                    ...
                    <th class="text-left" style="width: 25%; padding: 10px;">
                        <p style="font-size: 14px; margin: 5px 0;"><b>Total Qty: </b><b>' . $total_qty . '</b></p>
                </th>
                    <th class="text-left" style="width: 25%; padding: 10px;">
            ...
            <table id="invoice_3" class="m-t-10 table table-bordered" style="table-layout: fixed;">
                ...
                <tr>
                    <th class="p-l-r text-left" style="width: 36%;">
                        <p><b>Sold By: </b>' . $seller_name . '</p>
                        <p><b>Address: </b>' . $seller_address . $seller_pincode . '</p>
                        <p class="m-t-10"><b>PAN: </b>' . $seller_pan . '</p>
                        <p><b>GSTIN: </b>' . $seller_gstin . '</p>
                        <p><b>Place of Supply: </b>' . $seller_state . '</p>
                    </th>
                    <th class="p-l-r text-left"  style="width: 36%;">
