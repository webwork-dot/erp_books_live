...
class Helpers
{
    ...
    {
        // PHP 8.3 compatibility: ensure parameters are not null
        $protocol = $protocol !== null ? mb_strtolower($protocol, "UTF-8") : '';
        $host = $host !== null ? $host : '';
        $base_path = $base_path !== null ? $base_path : '';
        $url = $url !== null ? $url : '';
    ...
    public static function explode_url($url)
    {
        ...
        // PHP 8.3 compatibility: ensure $url is not null
        if ($url === null) {
            $url = '';
        }
        ...
        if (isset($arr["scheme"]) && $arr["scheme"] !== "file" && $arr["scheme"] !== "phar" && strlen($arr["scheme"]) > 1) {
            ...
            $i = $url !== null && $url !== '' ? mb_stripos($url, "://", 0, "UTF-8") : false;
            if ($i !== false) {
    ...
    public static function getFileContent($uri, $context = null, $offset = 0, $maxlen = null)
    {
        ...
        try {
            if ($is_local_path || ini_get('allow_url_fopen') && !$can_use_curl) {
                ...
                // PHP 8.3 compatibility: don't pass null as context parameter
                $stream_context = $context !== null ? $context : stream_context_create([]);
                if (isset($maxlen)) {
                    $result = file_get_contents($uri, false, $stream_context, $offset, $maxlen);
                } else {
                    $result = file_get_contents($uri, false, $stream_context, $offset);
                }
                if ($result !== false) {
                    $content = $result;
                }
