...
/**
 * Vendor POS School UPI QR Controller
 */
class Pos_school_qr extends Vendor_base
{
    ...
    public function index()
    {
        ...
        $data['content'] = $this->load->view('vendor/pos_school_qr/index', $data, TRUE);
    ...
    public function add()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $data['content'] = $this->load->view('vendor/pos_school_qr/add', $data, TRUE);
            $this->load->view('vendor/layouts/index_template', $data);
            return;
        }
        ...
        if ($is_active) {
            $this->Pos_school_qr_model->activateForSchool($qr_id, $vendor_id, $school_id, $actor_id);
        }
    ...
    public function edit($id)
    {
        $id = (int)$id;
        $vendor_id = (int)$this->current_vendor['id'];
        $qr = $this->Pos_school_qr_model->getByIdForVendor($id, $vendor_id);
        if (!$qr) {
            show_404();
        }
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $data['content'] = $this->load->view('vendor/pos_school_qr/edit', $data, TRUE);
            $this->load->view('vendor/layouts/index_template', $data);
            return;
        }
        ...
        if (!empty($_FILES['qr_image']['name'])) {
            ...
            if ($upload['status'] !== 'success') {
                $this->session->set_flashdata('error', $upload['message']);
                redirect('pos-school-qr/edit/' . $id);
                return;
            }
        ...
        $updated = $this->Pos_school_qr_model->updateForVendor($id, $vendor_id, $update_data);
        ...
        if ($is_active) {
            $this->Pos_school_qr_model->activateForSchool($id, $vendor_id, $school_id, $actor_id);
        }
    ...
    public function activate($id)
    {
        ...
        if ($this->Pos_school_qr_model->activateForSchool($id, $vendor_id, (int)$row['school_id'], (int)$this->current_vendor['id'])) {
    ...
    public function delete($id)
    {
        ...
        if ($this->Pos_school_qr_model->deleteForVendor($id, $vendor_id)) {
