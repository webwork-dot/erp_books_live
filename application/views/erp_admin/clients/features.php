<h1>Manage Features for <?php echo htmlspecialchars($client['name']); ?></h1>

<?php echo form_open('erp-admin/clients/features/' . $client['id']); ?>
    <div class="card">
        <div class="card-body">
            <table class="table">
                <thead>
                ...
                <tbody>
                    <?php foreach ($all_features as $feature): ?>
                        <?php
                        $enabled = FALSE;
                        foreach ($client_features as $cf) {
                            if ($cf['id'] == $feature['id'] && $cf['is_enabled']) {
                                $enabled = TRUE;
                                break;
                            }
                        }
            ...
            <div style="display: flex; gap: 0.75rem; margin-top: 1.5rem;">
                ...
                <a href="<?php echo base_url('erp-admin/clients'); ?>" class="btn btn-secondary">Back to Clients</a>
            </div>
        </div>
    </div>
<?php echo form_close(); ?>
