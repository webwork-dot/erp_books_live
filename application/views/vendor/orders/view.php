...
if ($order_data[0]->payment_status == 'pending') {
  ...
  switch ($order_data[0]->order_status) {
    ...
    case '6':
      ...
      $status_text = 'Ready for Shipment';
      break;
    case '3':
      $status_badge = 'badge-primary';
...
<div class="container-fluid order-page" style="padding: 20px;">
  ...
  <div class="row">
                          ...
                          <tr>
                            ...
                            <td class="text-end">
                              ...
                              <?php if ($book['quantity'] > 1): ?>
                                <br><small class="text-muted">(<?= $currency_code . ' ' . number_format($book['total_price'], 2) ?>
                                  total)</small>
                                <?php
                              endif; ?>
                            </td>
                      ...
                      foreach ($package_data['products'] as $bookset_product) {
                        ...
                        <tr>
                          <td>
                            <?php if (!empty($product_image)):
                              ...
                              <img src="<?= $img_url ?>" class="product-image"
                                onerror="this.onerror=null; this.src='<?php echo base_url('assets/template/img/placeholder-image.png'); ?>';" />
                              <?php
                            else: ?>
                          ...
                          <td class="text-center">
                            <?= !empty($bookset_product->product_sku) ? htmlspecialchars($bookset_product->product_sku) : '-' ?>
                          </td>
                    ...
                    <tr>
                      <td colspan="5" style="background-color: #e8f4f8; padding: 15px;">
                        ...
                        <?php if (!empty($bookset_info->f_name) || !empty($bookset_info->m_name) || !empty($bookset_info->s_name)): ?>
                          ...
                          <?= trim(htmlspecialchars((isset($bookset_info->f_name) ? $bookset_info->f_name : '') . ' ' . (isset($bookset_info->m_name) ? $bookset_info->m_name : '') . ' ' . (isset($bookset_info->s_name) ? $bookset_info->s_name : ''))) ?><br>
                          <?php
                        endif; ?>
    ...
    <div class="col-md-4 sticky-sidebar">
        ...
        <div class="card mb-3">
          ...
          <div class="card-body py-2 small">
            ...
            <?php if (!empty($order_data[0]->track_url)):
              ...
              ?>
              <div class="mt-1">
                <a href="<?= htmlspecialchars($track_url_href) ?>" target="_blank" rel="noopener" class="text-break"
              ...
              <?php if (!empty($uniform_student_details)): ?>
                <?php foreach ($uniform_student_details as $stu): ?>
                  <div class="mb-3 p-2 rounded" style="background: #fff3cd; border: 1px solid #ffc107;">
                    <?php if (!empty($stu->f_name)): ?>
                      <div><strong>Name:</strong> <?= htmlspecialchars($stu->f_name) ?></div><?php
                    endif; ?>
                    <?php if (!empty($stu->grade)): ?>
                      <div><strong>Grade:</strong> <?= htmlspecialchars($stu->grade) ?></div><?php
                    endif; ?>
                    <?php if (!empty($stu->roll_number)): ?>
                      <div><strong>Roll Number:</strong> <?= htmlspecialchars($stu->roll_number) ?></div><?php
                    endif; ?>
                    <?php if (!empty($stu->remarks)): ?>
                      <div><strong>Remarks:</strong> <?= htmlspecialchars($stu->remarks) ?></div><?php
                    endif; ?>
                  </div>
                  <?php
                endforeach; ?>
      ...
      <!-- SOURCE INFO CARD -->
      <?php if (isset($order_data[0]->source) && $order_data[0]->source == 'app'): ?>
        <div class="card mb-3">
          <div class="card-header">
            <b><i class="fa fa-info-circle me-1"></i>App Source Details</b>
          </div>
          <div class="card-body">
            <div class="d-flex flex-column gap-2">
              <div>
                <span class="badge bg-primary text-white">Source: Mobile App</span>
              </div>
              <?php if (!empty($agent_name)): ?>
                <div>
                  <i class="fa fa-user-circle me-1 text-muted"></i>
                  <strong>Agent:</strong> <?= htmlspecialchars($agent_name) ?>
                </div>
              <?php endif; ?>
              <?php if (!empty($app_school_name)): ?>
                <div>
                  <i class="fa fa-school me-1 text-muted"></i>
                  <strong>School:</strong> <?= htmlspecialchars($app_school_name) ?>
                </div>
              <?php endif; ?>
            </div>
          </div>
      ...
      <div class="card mb-3">
        ...
        <div class="card-body">
          <div>
            <b><?= htmlspecialchars(isset($address_arr[0]->name) ? $address_arr[0]->name : $order_data[0]->user_name) ?></b>
          </div>
          <div class="text-muted">
            <?= htmlspecialchars(isset($address_arr[0]->mobile_no) ? $address_arr[0]->mobile_no : $order_data[0]->user_phone) ?>
          </div>
        </div>
      </div>
      ...
      <?php if (!empty($children_data)): ?>
        ...
        <div class="card mb-3">
          <div class="card-header">
            <b><i class="fa fa-child me-1"></i>Children Details</b>
          </div>
          <div class="card-body p-0">
            <?php foreach ($children_data as $i => $child): ?>
              <div class="d-flex align-items-center px-3 py-2 <?= $i > 0 ? 'border-top' : '' ?>">
                <div class="me-3 text-center"
                  style="width:32px;height:32px;border-radius:50%;background:#e8f0fb;display:flex;align-items:center;justify-content:center;">
                  <small class="fw-bold text-primary"><?= $i + 1 ?></small>
                </div>
                <div>
                  <div class="fw-bold text-dark">
                    <?= htmlspecialchars(isset($child['name']) ? $child['name'] : (isset($child['childName']) ? $child['childName'] : '—')) ?>
                  </div>
                  <?php if (!empty($child['grade'])): ?>
                    <div class="text-muted small"><i
                        class="fa fa-graduation-cap me-1"></i><?= htmlspecialchars($child['grade']) ?></div>
                    <?php
                  endif; ?>
                </div>
              </div>
            ...
            <div class="mt-2">
              ...
              if ($use_school_address) {
                ...
                if (!empty($addr->address))
                  $address_parts[] = htmlspecialchars($addr->address);
                if (!empty($addr->city))
                  $address_parts[] = htmlspecialchars($addr->city);
                if (!empty($addr->state))
                  $address_parts[] = htmlspecialchars($addr->state);
                if (!empty($addr->pincode))
                  $address_parts[] = htmlspecialchars($addr->pincode);
                if (!empty($addr->country))
                  $address_parts[] = htmlspecialchars($addr->country);
        ...
        if (!empty($status_history)) {
          foreach ($status_history as $history) {
            if ($history->status_type == 'order_status') {
              ...
              foreach ($timeline_items as $item) {
                if (
                  stripos($item['status'], $status_label) !== false ||
...
<div class="modal fade" id="thirdPartyShippingModal" tabindex="-1" aria-labelledby="thirdPartyShippingModalLabel"
  aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-lg">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <button type="button" class="btn btn-primary" id="saveThirdPartyBtn" disabled
          onclick="saveThirdPartyShipping()">
          <span id="saveBtnText">
...
<div class="modal fade" id="selectCourierModal" tabindex="-1" aria-labelledby="selectCourierModalLabel"
  aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-lg">
    <div class="modal-content">
      <div class="modal-header">
      ...
      <div class="modal-body">
        <div id="courierStep1" class="courier-step">
          ...
          <div id="courierListContainer" class="list-group">
            ...
            <div id="courierListEmpty" class="alert alert-warning" style="display: none;">No couriers found. Please add
              couriers in <a href="<?php echo base_url('couriers'); ?>">Couriers</a> first.</div>
          </div>
        </div>
        <div id="courierStep2" class="courier-step mt-3" style="display: none;">
          <hr>
          <p class="text-muted mb-2">AWB / Tracking Number <small class="text-muted">(optional - leave blank to
              skip)</small>:</p>
          <input type="text" id="awbNumberInput" class="form-control form-control-lg" placeholder="e.g. 123456789012"
            maxlength="50">
