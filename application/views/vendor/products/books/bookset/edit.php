		...
		<div class="card">
			<div class="card-body">
				...
				<div id="packages_help_text" class="mt-3 text-center" style="display: none;">
					<small class="text-muted">Packages are displayed one by one. Each package has an "Add Next Package" button at the bottom.</small>
				</div>
			</div>
		</div>
	</div>
...
(function() {
	function initScript() {
		...
		if (packages.length > 0) {
			...
			// After loading all existing packages, hide "Add Next Package" button from all except the last one
			setTimeout(function() {
				$('.add-next-package-btn').hide();
				$('.package-card').last().find('.add-next-package-btn').show();
			}, 200);
		} else {
			// Don't initialize first package automatically - show initial button instead
			updatePackageSummary();
		}
	}
