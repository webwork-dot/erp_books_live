...
<div class="d-flex d-block align-items-center justify-content-between flex-wrap gap-3 mb-3">
	<div>
		<h6><a href="<?php echo base_url(isset($current_vendor['domain']) ? $current_vendor['domain'] . '/products/bookset?tab=without_product' : 'products/bookset?tab=without_product'); ?>"><i class="isax isax-arrow-left me-2"></i>Edit Bookset</a></h6>
	</div>
</div>
<!-- End Breadcrumb -->
...
<?php echo form_open(isset($current_vendor['domain']) ? base_url($current_vendor['domain'] . '/products/bookset/edit/' . $bookset['id']) : base_url('products/bookset/edit/' . $bookset['id']), array('id' => 'bookset-without-products-form')); ?>
								...
								<select name="school_id" id="school_id" class="select" required>
									...
									<?php if (!empty($schools)): ?>
										<?php foreach ($schools as $school): ?>
											<option value="<?php echo $school['id']; ?>" <?php echo (isset($bookset['school_id']) && $bookset['school_id'] == $school['id']) ? 'selected' : ''; ?>><?php echo htmlspecialchars($school['school_name']); ?></option>
										<?php endforeach; ?>
									<?php endif; ?>
								</select>
	...
	function initScript() {
		...
		window.addPackage = function(existingPackage) {
					...
					<div class="card-body p-3">
						<div class="row g-2 mb-3">
							...
							<div class="col-md-2 col-sm-6">
								...
								<input type="number" class="form-control form-control-sm package-price-input" data-package-id="${packageCounter}" step="0.01" min="0" placeholder="0.00" required value="${existingPackage ? (existingPackage.package_price || '') : ''}">
							</div>
							<div class="col-md-2 col-sm-6">
								<label class="form-label small fw-semibold mb-1">Offer Price (₹) <span class="text-danger">*</span></label>
								<input type="number" class="form-control form-control-sm package-offer-price-input" data-package-id="${packageCounter}" step="0.01" min="0" placeholder="0.00" required value="${existingPackage ? (existingPackage.package_offer_price || '') : ''}">
							</div>
							<div class="col-md-2 col-sm-6">
								<label class="form-label small fw-semibold mb-1">GST (%) <span class="text-danger">*</span></label>
								<input type="number" class="form-control form-control-sm package-gst-input" data-package-id="${packageCounter}" step="0.01" min="0" max="100" placeholder="0.00" required value="${existingPackage ? (existingPackage.gst || '') : ''}">
							</div>
							<div class="col-md-3 col-sm-6">
								<label class="form-label small fw-semibold mb-1">HSN</label>
								<input type="text" class="form-control form-control-sm package-hsn-input" data-package-id="${packageCounter}" placeholder="HSN code" value="${existingPackage ? (existingPackage.hsn || '') : ''}">
							</div>
						</div>
						<div class="row g-2 mb-3">
			...
			var packageObj = {
				...
				package_price: existingPackage ? (existingPackage.package_price || '') : '',
				package_offer_price: existingPackage ? (existingPackage.package_offer_price || '') : '',
				gst: existingPackage ? (existingPackage.gst || '') : '',
				hsn: existingPackage ? (existingPackage.hsn || '') : '',
