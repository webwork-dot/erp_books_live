...
<div class="d-flex d-block align-items-center justify-content-between flex-wrap gap-3 mb-3">
	...
	<div>
		<a href="<?php echo base_url(isset($current_vendor['domain']) ? $current_vendor['domain'] . '/products/textbook/add' : 'products/textbook/add'); ?>" class="btn btn-primary">
...
<div class="card mb-3">
	<div class="card-body">
		<form method="get" action="<?php echo base_url(isset($current_vendor['domain']) ? $current_vendor['domain'] . '/products/textbook' : 'products/textbook'); ?>">
			<!-- Search on Top -->
				...
				<div class="col-lg-2 col-md-4">
					<div class="mb-3">
						...
						<div class="d-flex gap-2">
							...
							<a href="<?php echo base_url(isset($current_vendor['domain']) ? $current_vendor['domain'] . '/products/textbook' : 'products/textbook'); ?>" class="btn btn-outline-secondary">Clear</a>
						</div>
					</div>
				</div>
			</div>
					...
					<?php if (!empty($textbook_list)): ?>
						<?php foreach ($textbook_list as $textbook): ?>
							<tr>
								...
								<td class="text-end">
									<a href="<?php echo base_url(isset($current_vendor['domain']) ? $current_vendor['domain'] . '/products/textbook/edit/' . $textbook['id'] : 'products/textbook/edit/' . $textbook['id']); ?>" class="btn btn-sm btn-outline-primary" data-bs-toggle="tooltip" title="Edit">
										<i class="isax isax-edit"></i>
									</a>
									<a href="<?php echo base_url(isset($current_vendor['domain']) ? $current_vendor['domain'] . '/products/textbook/delete/' . $textbook['id'] : 'products/textbook/delete/' . $textbook['id']); ?>" onclick="return confirm('Are you sure you want to delete this textbook?');" class="btn btn-sm btn-outline-danger" data-bs-toggle="tooltip" title="Delete">
					...
					<nav aria-label="Page navigation">
						<ul class="pagination pagination-sm mb-0">
							<?php if ($current_page > 1): ?>
								<li class="page-item">
									<a class="page-link" href="<?php echo base_url(isset($current_vendor['domain']) ? $current_vendor['domain'] . '/products/textbook?' . http_build_query(array_merge($filters, array('page' => $current_page - 1))) : 'products/textbook?' . http_build_query(array_merge($filters, array('page' => $current_page - 1)))); ?>">Previous</a>
								</li>
							<?php else: ?>
							...
							<?php for ($i = 1; $i <= $total_pages; $i++): ?>
								...
								<?php else: ?>
									<li class="page-item">
										<a class="page-link" href="<?php echo base_url(isset($current_vendor['domain']) ? $current_vendor['domain'] . '/products/textbook?' . http_build_query(array_merge($filters, array('page' => $i))) : 'products/textbook?' . http_build_query(array_merge($filters, array('page' => $i)))); ?>"><?php echo $i; ?></a>
									</li>
								<?php endif; ?>
							<?php endfor; ?>
							...
							<?php if ($current_page < $total_pages): ?>
								<li class="page-item">
									<a class="page-link" href="<?php echo base_url(isset($current_vendor['domain']) ? $current_vendor['domain'] . '/products/textbook?' . http_build_query(array_merge($filters, array('page' => $current_page + 1))) : 'products/textbook?' . http_build_query(array_merge($filters, array('page' => $current_page + 1)))); ?>">Next</a>
								</li>
							<?php else: ?>
								<li class="page-item disabled">
